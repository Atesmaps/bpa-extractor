...
def accept_cookies_policy(driver):
    """
    Accept coolies policy if it's needed.
    ...
    try:
        ...
        print(
            "Acceptance of the cookie policy is not required or has already been accepted previously."
        )


def get_danger_level_by_zone(driver, date: str):
    """
    Return danger level for each zone and date selected.
    ...
    for area in avalancheIconElements:
        ...
        pos_matrix = (
            css_pos_matrix[
                css_pos_matrix.find("(") + 1 : css_pos_matrix.find(")")  # noqa: E203
            ]
            .replace(" ", "")
            .split(",")[-2:]
        )
        ...
        if not zone_name:
            print(
                f"Skipping zone with coordinates x:{pos_matrix[0]} and y:{pos_matrix[1]} because is not declared."
            )
            continue
...
def main():
    ...
    print("Zone: MeteoFrance - Pyrenees Français")
    print(f"Date: {today}")
